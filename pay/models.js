...
export function normalizeCampaign(raw) {
  ...
  return Object.freeze({
    ...
    bundlePricing: raw.bundlePricing ? Object.freeze({ quantity: nonNegativeInteger(raw.bundlePricing.quantity, "bundlePricing.quantity"), price: nonNegativeInteger(raw.bundlePricing.price, "bundlePricing.price") }) : null,
    products: Object.freeze(products),
  });
}

export function calculateSelection(campaign, quantities) {
  const summary = campaign.products.reduce((summary, product) => {
  ...
  if (campaign.bundlePricing?.quantity >= 2 && summary.totalQuantity >= campaign.bundlePricing.quantity) {
    const prices = new Set(campaign.products.filter((product) => quantities[product.id] > 0).map((product) => product.unitPrice));
    if (prices.size === 1) {
      const unitPrice = [...prices][0];
      const discount = Math.floor(summary.totalQuantity / campaign.bundlePricing.quantity) * Math.max(0, unitPrice * campaign.bundlePricing.quantity - campaign.bundlePricing.price);
      summary.totalAmount -= discount;
    }
  }
  return summary;
}
